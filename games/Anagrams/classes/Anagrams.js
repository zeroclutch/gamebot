...
// Define common words
let commonWords = fs.readFileSync('./gameData/WordGames/Common_Words.txt', { encoding: 'utf-8' }, err => {
    logger.error(err)
})

commonWords = commonWords.split('\n')
commonWords.splice(0,1)
...
export default class Anagrams extends Game {
     ...
     */
    chooseWord(length) {
        const validWords = commonWords.filter(w => w.length == length)
